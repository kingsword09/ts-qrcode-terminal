...
import { QRErrorCorrectLevel } from "./constants.ts";
export { QRErrorCorrectLevel } from "./constants.ts";
...
export class QRCode {
  ...
  static createData(
    ...
    const rsBlocks: QRRSBlock[] = QRRSBlock.getRSBlocks(
      typeNumber,
      errorCorrectLevel
    );
    const buffer = new QRBitBuffer();
  ...
  static createBytes(buffer: QRBitBuffer, rsBlocks: QRRSBlock[]) {
    ...
    for (let r = 0; r < rsBlocks.length; r++) {
      ...
      const rawPoly: QRPolynomial = new QRPolynomial(
        dcdata[r],
        rsPoly.getLength() - 1
      );
